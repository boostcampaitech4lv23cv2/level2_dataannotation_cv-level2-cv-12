...
parser.add_argument('--valid_json', type=str, default='/opt/ml/valid_illegibility.json', help='Infered된 json 파일의 경로 ex)~/submission.json')
...
def draw_bbox(img, word_idx, points, color, thickness):
    ...
    textX = points[0]
    textY = points[3] - int(h * TEXT_Y_OFFSET_SCALE)
...
def main():
    ...
    precision_range = st.sidebar.slider(f'이미지 recall 범위', min_value=0., max_value=1., value=(0., 1.), step=0.01, key='precision')
    ...
    for i, bbox in enumerate(gt_bboxes):
        ...
        if check:
            gt_bboxes = pred_data['per_sample'][img_filename]['gt_bboxes']
            draw_bbox(img, str(i), bbox, (255, 0, 0), 3)
